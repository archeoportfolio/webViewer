...
// Add mouseenter and mouseleave event listeners for automatic light mode
            var actualCanvasElement = document.getElementById(canvasName);
            var lightButton = $( "#"+idDiv + "_div #light" ); 

            if (actualCanvasElement && lightButton.length) {
                actualCanvasElement.addEventListener('mouseenter', function(event) {
                    // Activate light mode only if multiResRTI is initialized, not animating, and it's an RTI (not a plain image)
                    if (multiResRTI && multiResRTI.renderer && multiResRTI.renderer.type !== "IMAGE" && !multiResRTI.animating) { 
                        multiResRTI.setMode(1);
                        lightButton.button( "option", {
                            label: "Light On",
                            icons: { primary: "lightOnIcon toolbarIcon" }
                        });
                        if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[canvasName]) {
                           _SGL_RegisteredCanvas[canvasName].requestDraw();
                        }
                    }
                });

                actualCanvasElement.addEventListener('mouseleave', function(event) {
                    // Deactivate light mode if multiResRTI is initialized and it's an RTI
                    if (multiResRTI && multiResRTI.renderer && multiResRTI.renderer.type !== "IMAGE") { 
                        multiResRTI.setMode(0);
                        lightButton.button( "option", {
                            label: "Light Off",
                            icons: { primary: "lightIcon toolbarIcon" }
                        });
                        if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[canvasName]) {
                            _SGL_RegisteredCanvas[canvasName].requestDraw();
                        }
...
MultiRes.prototype = {
	...
	{
		return; // Disabled zoom animation
	...
	mouseDown : function(gl, button, x, y)
	{
		...
		{
			// Ctrl key + click for light manipulation in camera mode
			if (this.ui.keysDown[17] && this.renderer && this.renderer.type !== "IMAGE")
			{
				this._setLightDir(x, y);
				if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[this.canvas]) {
					_SGL_RegisteredCanvas[this.canvas].requestDraw();
				}
				return true;
			}

			if (this.mode == 0) // Camera mode (pan)
			{
				// Panning disabled
				return false;
				/*
				...
				this.renderer.updateData = false;
				this.isMoving = true;
				return false; // Panning, so don't mark as handled for other potential listeners
				*/
			}
			else if (this.mode == 1 && this.renderer && this.renderer.type !== "IMAGE") // Light mode (click to set light)
			{
				this._setLightDir(x, y);
				if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[this.canvas]) {
					_SGL_RegisteredCanvas[this.canvas].requestDraw();
				}
				return true; // Light set, event handled
			}
		}
		return false; // Default: event not handled by this specific logic
	},
	...
	mouseMove: function(gl, x, y)
	{
		...
		// If in light mode (mode == 1, set by mouseenter) and it's an RTI image, update light direction on mouse move without click
		if (this.mode == 1 && this.renderer && this.renderer.type !== "IMAGE") {
			this._setLightDir(x, y);
			if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[this.canvas]) {
				_SGL_RegisteredCanvas[this.canvas].requestDraw();
			}
		...
		// Existing logic for mouse button down
		if (this.ui.mouseButtonsDown[0])
		{
			if (this.mode == 0) // Camera mode
			{
				// Panning disabled
				if (this.ui.keysDown[17]) // Ctrl key for light (still allow this)
				{
					this._setLightDir(x, y);
					if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[this.canvas]) {
						_SGL_RegisteredCanvas[this.canvas].requestDraw();
			...
			else if (this.mode == 1 && this.renderer && this.renderer.type !== "IMAGE") // Light mode (this handles when mouse button IS pressed in light mode)
			{
				this._setLightDir(x, y);
				// Ensure redraw if light is set this way
				if (_SGL_RegisteredCanvas && _SGL_RegisteredCanvas[this.canvas]) {
					_SGL_RegisteredCanvas[this.canvas].requestDraw();
				}
				return true;
